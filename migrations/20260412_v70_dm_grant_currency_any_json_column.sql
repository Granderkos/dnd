...
begin
  ...
  where c.table_schema = 'public'
    and c.table_name = 'character_notes'
  ...
  if v_notes_column is null then
    raise exception 'character_notes json column not found';
  end if;

  execute format('select coalesce(%I::jsonb, ''{}''::jsonb) from public.character_notes where character_id = $1', v_notes_column)
    into v_payload
